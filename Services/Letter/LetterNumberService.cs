    ...
    ILogger<LetterNumberService> logger) : ILetterNumberService
{
        ...
        try
        {
            ...
            return await strategy.ExecuteAsync(async () =>
            {
                ...
                try
                {
                    ...
                    logger.LogInformation("Generating formatted number for seq={Sequence}, company={CompanyCode}, docType={DocTypeCode}",
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(query.Search))
            {
                baseQuery = baseQuery.Where(l =>
                    l.FormattedNumber.Contains(query.Search) ||
                    l.Subject.Contains(query.Search) ||
                    l.Recipient.Contains(query.Search));
            }
            ...
            // Map to DTO in memory to avoid translation issues with Status.ToString()
            var items = data.Select(l => new LetterNumberListDto
            {
                Id = l.Id,
    ...
    {
        var l = await context.LetterNumbers
            .AsNoTracking()
            .Include(l => l.DocumentType)
            .Include(l => l.Company)
            .Include(l => l.CreatedByUser)
