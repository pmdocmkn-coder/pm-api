...
namespace Pm.Services
{
    public class CallRecordService : ICallRecordService
    {
        ...
        private async Task GenerateDailySummaryInternalAsync(AppDbContext context, DateTime date)
        {
            ...
            {
                // Race condition: request lain sudah generate summary → aman, skip saja
                _logger.LogWarning("Summary for {Date} already generated by another request, skipping", date.Date);
            }
        }
        ...
        public async Task<List<DateTime>> GetDatesWithDataAsync(int year, int month)
        {
            var startDate = new DateTime(year, month, 1);
            var endDate = startDate.AddMonths(1);

            var dates = await _context.CallRecords
                .Where(cr => cr.CallDate >= startDate && cr.CallDate < endDate)
